resource "azurerm_cdn_frontdoor_secret" "hrportal-certificate" {
...
resource "azurerm_cdn_frontdoor_custom_domain" "domain" {
  ...
  tls {
    ...
    cdn_frontdoor_secret_id = azurerm_cdn_frontdoor_secret.hrportal-certificate.id
  }
...
resource "azurerm_cdn_frontdoor_secret" "kb-certificate" {
  cdn_frontdoor_profile_id = var.afd_profile_id
...
resource "azurerm_cdn_frontdoor_custom_domain" "kb-domain" {
  ...
  tls {
    certificate_type        = "CustomerCertificate"
    cdn_frontdoor_secret_id = azurerm_cdn_frontdoor_secret.kb-certificate.id
  }
}
...
resource "azurerm_cdn_frontdoor_origin" "dnn" {
  for_each = {
    for index, vm in var.private_link_ids :
    vm.vm_name => vm
  }
  ...
  enabled                        = true
  host_name                      = var.host_name
  name                           = "${each.value.vm_name}-${var.env_name}"
  weight                         = 50
  private_link {
    location               = var.dnn_region
    private_link_target_id = each.value.private_link_id
    request_message        = "Embark Pro Environment: ${var.env_name}, VM: ${each.value.vm_name}"
  }
}

resource "azurerm_cdn_frontdoor_origin_group" "boost" {
  cdn_frontdoor_profile_id                                  = var.afd_profile_id
  name                                                      = "BOOST-${var.env_name}"
  restore_traffic_time_to_healed_or_new_endpoint_in_minutes = 0
  session_affinity_enabled                                  = false
...
resource "azurerm_cdn_frontdoor_origin" "boost" {
  cdn_frontdoor_origin_group_id  = azurerm_cdn_frontdoor_origin_group.boost.id
  certificate_name_check_enabled = true
  enabled                        = var.enable_primary_region
  host_name                      = var.host_name
  priority                       = 1
  ...
  private_link {
    location               = var.dnn_region
    private_link_target_id = var.search_boost_private_link_id
    request_message        = "Embark Pro Environment: ${var.env_name}"
  }
}
...
resource "azurerm_cdn_frontdoor_route" "dnn" {
  ...
  cdn_frontdoor_endpoint_id       = azurerm_cdn_frontdoor_endpoint.endpoint.id
  cdn_frontdoor_origin_group_id   = azurerm_cdn_frontdoor_origin_group.dnn.id
  cdn_frontdoor_origin_ids        = [for o in azurerm_cdn_frontdoor_origin.dnn : o.id]
  cdn_frontdoor_rule_set_ids      = [azurerm_cdn_frontdoor_rule_set.ruleset.id]
  forwarding_protocol             = "HttpsOnly"
  link_to_default_domain          = false
  name                            = "DNN-${var.env_name}"
...
resource "azurerm_cdn_frontdoor_route" "boost" {
  cdn_frontdoor_custom_domain_ids = [azurerm_cdn_frontdoor_custom_domain.domain.id, azurerm_cdn_frontdoor_custom_domain.kb-domain.id]
  cdn_frontdoor_endpoint_id       = azurerm_cdn_frontdoor_endpoint.endpoint.id
  cdn_frontdoor_origin_ids        = [azurerm_cdn_frontdoor_origin.boost.id]
  cdn_frontdoor_origin_group_id   = azurerm_cdn_frontdoor_origin_group.boost.id
  cdn_frontdoor_rule_set_ids      = [azurerm_cdn_frontdoor_rule_set.ruleset.id]
  forwarding_protocol             = "HttpsOnly"
  link_to_default_domain          = false
  name                            = "Boost-${var.env_name}"
  patterns_to_match               = ["/DesktopModules/DnnSharp/SearchBoost/*", "/advocate/api/portal/embarksearch/monitor"]
  supported_protocols             = ["Http", "Https"]
  depends_on = [
    azurerm_cdn_frontdoor_endpoint.endpoint,
    azurerm_cdn_frontdoor_custom_domain.domain,
    azurerm_cdn_frontdoor_custom_domain.kb-domain,
    azurerm_cdn_frontdoor_origin_group.boost,
  ]
}
...
resource "azurerm_cdn_frontdoor_rule_set" "ruleset" {
  name                     = "${var.env_name}ServerHeaderRuleSet"
  cdn_frontdoor_profile_id = var.afd_profile_id
}
...
resource "azurerm_cdn_frontdoor_rule" "deleterule" {
  ...
  name                      = "${var.env_name}DeleteServerHeaders"
...
resource "azurerm_cdn_frontdoor_rule" "addrule" {
  ...
  name                      = "${var.env_name}AddSecurityHeaders"
  cdn_frontdoor_rule_set_id = azurerm_cdn_frontdoor_rule_set.ruleset.id
  order                     = 1
  behavior_on_match         = "Continue"
